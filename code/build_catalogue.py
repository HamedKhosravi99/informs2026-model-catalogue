...
def build():
    ...
    # Rows whose t01h field holds an exception string never produced a run at all -- the
    # process died on a code fault (a missing column, a None state_dict, an incompatible
    # tensor view). They are dropped: a crash is not an experiment.
    crashed = d.rmse_t01h.astype(str).str.startswith("FAILED")
    n_crash = int(crashed.sum())
    d = d[~crashed].copy()
    num = pd.to_numeric(d.rmse_mean, errors="coerce")
    # SARIMAX is different: it ran for 9 s and produced output, then diverged to a
    # magnitude of 1e80. That IS a result -- the method fails on this data -- so the run
    # stays in the record with its unusable score blanked.
    d.loc[num > 0.1, HCOLS] = np.nan
    d.loc[num > 0.1, "diverged"] = True
    for c in HCOLS:
        d[c] = pd.to_numeric(d[c], errors="coerce")
    print(f"dropped {n_crash} crashed runs (exception, no output); "
          f"{int(pd.to_numeric(d.rmse_mean, errors='coerce').isna().sum())} run(s) kept with an unusable score")
...
def to_markdown(d, path):
    L = ["# Experiment catalogue", "",
         f"Every run in this project that produced a result: **{len(d)} experiments**, each classified by",
